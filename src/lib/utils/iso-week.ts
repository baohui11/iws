...
export function parseWeekCode(code: string): { year: number; week: number } | null {
...
/** 从 endCode 起（含）向过去取 count 个周次，降序 [最新, …] */
export function listWeekCodesDescending(endCode: string, count: number): string[] {
  const codes: string[] = []
...
/** 升序区间 [startCode, endCode]（含），要求均为合法周次且 start <= end */
export function weekCodesInclusiveRange(startCode: string, endCode: string): string[] {
