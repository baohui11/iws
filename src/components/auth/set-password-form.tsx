...
import { setPassword } from "@/actions/auth/set-password";

addCollection(solarIcons as Parameters<typeof addCollection>[0]);

export interface SetPasswordFormProps {
  /** page：独立页样式；embedded：个人设置内嵌 */
  variant?: "page" | "embedded";
  /** embedded 且修改成功时回调（例如收起区块） */
  onSuccess?: () => void;
}
...
export default function SetPasswordForm({
  ...
  const pageWrapper =
    "rounded-large bg-background/60 shadow-small dark:bg-default-100/50 flex w-full max-w-sm flex-col gap-4 px-8 pt-6 pb-10 backdrop-blur-md backdrop-saturate-150 -mt-12";
  ...
  return (
    <div className={isEmbedded ? "w-full max-w-md" : pageWrapper}>
      {!isEmbedded ? (
        ...
        <p className="text-default-600 mb-1 text-small">
          新密码至少 8 位，请两次输入一致。
        </p>
      )}
