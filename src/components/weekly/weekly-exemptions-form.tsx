...
import {
  addProjectWeekExemption,
  removeProjectWeekExemption,
} from '@/actions/weekly/exemptions.action'
import type {
  MemberProjectOption,
...
export default function WeeklyExemptionsForm({
  ...
  const handleSubmit = () => {
    ...
    startTransition(async () => {
      const result = await addProjectWeekExemption({
        projectId,
        weekCode,
      })
      if (result.success) {
  ...
  const handleDelete = (id: string) => {
    startTransition(async () => {
      const result = await removeProjectWeekExemption(id)
      if (result.success) {
