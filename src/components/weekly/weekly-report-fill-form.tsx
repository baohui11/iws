...
} from '@/actions/weekly/report-editor.action'
import WeeklyReportDeliverablePickerModal from '@/components/weekly/weekly-report-deliverable-picker-modal'
import {
...
import { formatWeekRangeLine, formatWeekTitleZh } from '@/lib/utils/week-display'
...
import type { WeeklyReportEditorPayload } from '@/types/weekly-report-editor'
...
export default function WeeklyReportFillForm({
  ...
  const openPickerForRow = (clientKey: string) => {
    setPickerTargetKey(clientKey)
    setPickerOpen(true)
  }
        ...
        <CardBody className="gap-3 p-3 sm:p-4">
          ...
          {section === 'work' ? (
            <div className="flex flex-wrap items-center gap-2">
              {row.files.length === 0 ? (
                <span className="text-xs text-default-400">未关联成果文件</span>
              ) : (
              ...
              <Button
                ...
                onPress={() => openPickerForRow(row.clientKey)}
              >
                关联成果文件
              </Button>
            </div>
  ...
  const pickerRow =
    [...workRows, ...planRows].find((r) => r.clientKey === pickerTargetKey) ??
    null
  ...
  return (
    <div className="space-y-4 pb-24 sm:pb-8">
      ...
      <WeeklyReportDeliverablePickerModal
        isOpen={pickerOpen}
        onClose={() => {
          setPickerOpen(false)
        ...
        onConfirm={(ids, picked) => {
          ...
          if (!targetKey) return
          const fromPicker = new Map(picked.map((p) => [p.id, p.file_name]))
          const patchFiles = (r: FillRow): FillRow => {
            if (r.clientKey !== targetKey) return r
            const files = ids.map((id) => ({
              id,
              file_name:
                fromPicker.get(id) ??
                r.files.find((f) => f.id === id)?.file_name ??
                '（已选文件）',
            }))
            return { ...r, file_ids: ids, files }
          }
          setWorkRows((prev) => prev.map(patchFiles))
          setPickerOpen(false)
          setPickerTargetKey(null)
          setTimeout(() => {
            const row = findRow(targetKey)
            if (!row) return
            const section = row.item_type === 'work' ? 'work' : 'plan'
            void saveRowInternal(targetKey, section, { silent: true })
          }, 0)
        }}
      />
    </div>
