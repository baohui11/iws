...
import {
  loadFileDownloadByPerson,
  loadFileDownloadDetails,
} from '@/actions/stats/file-download-audit.action'
...
export default function FileDownloadAuditClient() {
  ...
  const fetchPerson = useCallback(async () => {
    ...
    const res = await loadFileDownloadByPerson({
      dateFrom,
      dateTo,
  ...
  const fetchDetails = useCallback(
    async (page: number) => {
      ...
      const res = await loadFileDownloadDetails({
        dateFrom,
        dateTo,
        nameKeyword: nameKw.trim() || null,
        offset,
