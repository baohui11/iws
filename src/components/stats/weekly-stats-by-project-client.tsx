...
import { loadWeeklyDeptByProject } from '@/actions/stats/weekly-stats.action'
import type { WeeklyDeptByProjectRow } from '@/types/stats'
...
export default function WeeklyStatsByProjectClient({
  ...
  const runQuery = useCallback(async (s: WeeklyStatsFiltersState) => {
    setLoading(true)
    const result = await loadWeeklyDeptByProject(
      s.departmentId,
      s.weekCode,
