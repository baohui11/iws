...
import { createProject, updateProject } from '@/actions/admin/projects.action'
...
import { randomClientId } from '@/lib/utils/random-client-id'
...
export default function ProjectForm({ mode, project, departments, users }: ProjectFormProps) {
  ...
  useEffect(() => {
    ...
    setProjectStage(project?.project_stage?.trim() ?? '')
    setStatus(new Set(project?.project_status ? [project.project_status] : []))
    setMembers(
      project?.members?.length
        ? project.members.map((m) => ({
            key: m.id,
  ...
  const roleOptionsForStage = useMemo(() => {
    const roles = projectRolesForStage(projectStage)
    return roles.map((v) => ({ key: v, label: PROJECT_ROLE_LABEL[v] }))
  }, [projectStage])

  const addMember = () => {
    setMembers((prev) => [...prev, { key: randomClientId(), user_id: '', project_role: '' }])
  }
  ...
  const patchMember = (key: string, patch: Partial<Pick<MemberLine, 'user_id' | 'project_role'>>) => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const base = {
      ...
      project_status: (selectionToString(status) || null) as ProjectDetail['project_status'],
  ...
  return (
    <Form
      ...
      className="flex grid grid-cols-12 flex-col gap-6 py-8 justify-center items-end"
    >
      ...
      <Input
        ...
        placeholder="请输入项目编号"
        variant="underlined"
        defaultValue={project?.project_no ?? ''}
      ...
      <Input
        ...
        type="date"
        variant="underlined"
        defaultValue={dateInputValue(project?.start_date)}
        isDisabled={isPending}
      />
      <Input
        className="col-span-12 md:col-span-6"
        ...
        type="date"
        variant="underlined"
        defaultValue={dateInputValue(project?.end_date)}
        isDisabled={isPending}
      />
      <Input
        className="col-span-12 md:col-span-6"
      ...
      <Input
        ...
        placeholder="请输入行业分类"
        variant="underlined"
        defaultValue={project?.industry_category ?? ''}
        isDisabled={isPending}
      />
      <Input
        className="col-span-12 md:col-span-6"
        ...
        placeholder="请输入产品板块"
        variant="underlined"
        defaultValue={project?.product_block ?? ''}
        isDisabled={isPending}
      />
      <Textarea
        ...
        variant="underlined"
        minRows={3}
        defaultValue={project?.project_introduction ?? ''}
        isDisabled={isPending}
      />
      ...
      <div className="col-span-12 space-y-4">
        {members.length === 0 && (
          <p className="text-sm text-default-500">暂无成员，可点击「添加成员」。</p>
        )}
        {members.map((m) => (
          <div
            key={m.key}
            className="flex flex-col gap-4 rounded-lg bg-default-50 p-4 md:flex-row md:items-end"
          >
            <Select
              label="用户"
              ...
              onSelectionChange={(sel) => {
                const id = selectionToString(sel)
                patchMember(m.key, { user_id: id })
              }}
              variant="underlined"
              isDisabled={isPending}
              placeholder="请选择用户"
            ...
            <Select
              ...
              onSelectionChange={(sel) => {
                const id = selectionToString(sel)
                patchMember(m.key, { project_role: parseProjectRole(id) ?? '' })
              }}
              variant="underlined"
              isDisabled={isPending}
              placeholder="请选择角色"
      ...
      <div className="col-span-12 space-y-4">
        ...
        {deliverables.map((d) => (
          <div
            key={d.key}
            className="grid grid-cols-1 gap-4 rounded-lg bg-default-50 p-4 md:grid-cols-12 md:items-end"
          >
            <Input
              ...
              placeholder="成果名称"
              value={d.name}
              onValueChange={(v) => patchDeliverable(d.key, { name: v })}
              variant="underlined"
              isRequired
              isDisabled={isPending}
            />
            <Input
              ...
              placeholder="可选说明"
              value={d.description}
              onValueChange={(v) => patchDeliverable(d.key, { description: v })}
              variant="underlined"
              isDisabled={isPending}
            />
            <div className="flex items-end justify-end md:col-span-1">
              <Button
                ...
                color="danger"
                aria-label="移除"
                onPress={() => removeDeliverable(d.key)}
                isDisabled={isPending}
              >
                <Icon icon="solar:trash-bin-trash-linear" className="text-lg" />
              </Button>
