...
export default function DepartmentForm({
  ...
  const [parentKey, setParentKey] = useState<Selection>(
    () => new Set([initial?.parent_id ?? ROOT_KEY])
  )
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      if (mode === 'create') {
        const result = await createDepartment({ code, name, parent_id })
        if (result.success) {
      ...
      if (!initial) return
      const result = await saveDepartment({ id: initial.id, code, name, parent_id })
      if (result.success) {
